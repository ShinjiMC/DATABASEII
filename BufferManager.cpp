...
std::vector<int> BufferManager::findIndicesOf(const std::vector<std::string>& strings, std::string n) {
...
void BufferManager::CargarArchivoVariable(const char* filename) {
    ...
    int idKey = 1;
    for (auto cadenaR : valores) {
        std::cout << idKey << std::endl;
        AllPages.AddNewRegVar(MAXREG, idKey, cadenaR,false);
        idKey++;
...
void BufferManager::AddRegistro(const char* tabla, int numReg, std::string registro) {
    ...
    if (j == 0) {
        ...
        registro += "|";
        AllPages.AddNewRegVar(MAXREG, numReg, registro,true);
    }
    else {
        std::string registroCompleto;
        int i2 = 0;
